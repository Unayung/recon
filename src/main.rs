...
fn run_tui(start_mode: ViewMode, keep: bool) -> io::Result<()> {
    enable_raw_mode()?;
    ...
    let result = run_app(&mut terminal, start_mode, keep);
...
fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, start_mode: ViewMode, keep: bool) -> io::Result<()> {
    let mut app = App::new();
